...
import { Utensils, ShoppingBag, ReceiptText, ChefHat } from 'lucide-react';
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#FFF8F2]/95 backdrop-blur-md border-t-2 border-[#111111] pb-safe sm:hidden">
      <div className="flex items-center justify-around h-16 max-w-md mx-auto px-2">
        ...
        <Link
          ...
          className={`min-h-[44px] flex flex-col items-center justify-center flex-1 py-1 text-xs font-black transition-all ${
            pathname === '/' ? 'text-[#FF3B30] -translate-y-0.5' : 'text-[#475569] hover:text-[#111111]'
          }`}
          aria-label="Menu"
        >
        ...
        {isAdmin && (
          <Link
            href="/admin"
            className={`relative min-h-[44px] flex flex-col items-center justify-center flex-1 py-1 text-xs font-black transition-all ${
              pathname.startsWith('/admin') ? 'text-[#0F766E] -translate-y-0.5' : 'text-[#475569] hover:text-[#111111]'
            }`}
            aria-label="Kitchen Admin"
          >
            <div className="relative">
              <ChefHat className="w-5 h-5 mb-0.5 stroke-[2.5]" />
              {pendingKitchenOrders > 0 && (
                <span className="absolute -top-1.5 -right-2 bg-[#15803D] text-white text-[10px] font-black w-4 h-4 rounded-full border border-[#111111] flex items-center justify-center animate-pulse">
